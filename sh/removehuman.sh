...
function usage(){
	...
	echo "This script requires at least 24GB RAM."
	echo ""
...
function removehuman() {
	...
	#module load pigz
	local CMD="java $EA -Xmx23g -cp $CP align2.BBMap minratio=0.9 maxindel=3 bwr=0.16 bw=12 quickmatch fast minhits=2 path=/global/projectb/sandbox/gaag/bbtools/hg19 pigz unpigz zl=6 qtrim=rl trimq=10 untrim idtag sam=1.4 $@"
	echo $CMD >&2
	$CMD
}
