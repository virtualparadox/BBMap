...
usage(){
	...
	echo "Can also trim or mask the matching parts of reads."
	echo ""
	echo "Usage:	bbduk2.sh in=<input file> out=<output file> fref=<contaminant files>"
	echo ""
	...
	echo "fref=<file,file>  	Comma-delimited list of fasta reference files for filtering."
	echo "rref=<file,file>  	Comma-delimited list of fasta reference files for right-trimming."
	echo "lref=<file,file>  	Comma-delimited list of fasta reference files for left-trimming."
	echo "mref=<file,file>  	Comma-delimited list of fasta reference files for masking."
	echo "fliteral=<seq,seq>	Comma-delimited list of literal sequences for filtering."
	echo "rliteral=<seq,seq>	Comma-delimited list of literal sequences for right-trimming."
	echo "lliteral=<seq,seq>	Comma-delimited list of literal sequences for left-trimming."
	echo "mliteral=<seq,seq>	Comma-delimited list of literal sequences for masking."
...
#	echo "skipreads=0      	Ignore this many initial reads (or pairs) and process the rest."
	...
	echo "qhist=<file>		Write histogram of quality score by read location to file."
...
#	echo "                 	Any non-whitespace character other than t, f, r, l, n: convert to that symbol rather than trimming, and process short kmers on both ends."
	echo "kmask=f          	Convert bases matching the 'mref' kmers to this symbol.  't' and 'f' are not allowed, as they mean true and false.  Default is 'N'."
	echo "useshortkmers=f  	(usk) Look for shorter kmers at read tips (only for k-trimming/masking).  Enabling this will disable maskmiddle."
	echo "mink=6           	Minimum length of short kmers when trimming/masking.  Setting this automatically sets useshortkmers=t."
	...
	echo "minavgquality2=0 	(maq2) Averages probabilities rather than quality values."
...
bbduk2() {
	...
	local CMD="java $EA $z -cp $CP jgi.BBDuk2 $@"
	echo $CMD >&2
	$CMD
}
