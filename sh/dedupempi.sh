...
usage(){
	...
	echo "Last modified August 14, 2014"
	echo ""
...
#	echo "absorboverlap=f       (ao) Absorb (merge) non-contained overlaps of contigs (TODO)."
	...
	echo "usejni=f              Do alignments in C code, which is faster, if an edit distance is allowed."
...
dedupe() {
	...
	local CMD="mpirun -np 4 java -Djava.library.path=$NATIVELIBDIR $EA $z $z2 -cp $CP distributed.MPIDedupe $@"
	echo $CMD >&2
	$CMD
}
