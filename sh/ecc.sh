...
usage(){
	...
	echo ""
	echo "Usage:	ecc.sh in=<input> out=<reads to keep> outt=<reads to toss> hist=<histogram output>"
	echo ""
	...
	echo "aec=f			(aggressiveErrorCorrection) Sets more aggressive values of ecr=100, ecclimit=7, echt=16, eclt=3, pl=2."
...
correct() {
	...
	#module load pigz
	local CMD="java $EA $z -cp $CP jgi.KmerNormalize bits=16 ecc=t passes=1 keepall dr=f prefilter $@"
	echo $CMD >&2
	$CMD
}
