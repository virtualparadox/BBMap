...
public final class MultiStateAligner9fs {
	...
	public static void main(String[] args){
		...
		MultiStateAligner9fs msa=new MultiStateAligner9fs(read.length, ref.length, colorspace);
	...
	public MultiStateAligner9fs(int maxRows_, int maxColumns_, boolean colorspace_){
		maxRows=maxRows_;
		maxColumns=maxColumns_;
		colorspace=colorspace_;
		packed=new int[4][maxRows+1][maxColumns+1];
...
//		final int subfloor=Tools.max(BADoff, floor-200*POINTSoff_MATCH2);
		...
		for(int row=1; row<=rows; row++){
			...
			if(colStart>1){
				assert(row>0);
				packed[MODE_MATCH][row][colStart-1]=subfloor;
				packed[MODE_INS][row][colStart-1]=subfloor;
				packed[MODE_DEL][row][colStart-1]=subfloor;
				packed[MODE_SUB][row][colStart-1]=subfloor;
			}
...
//				System.err.println("")
				...
				final int scoreFromMatch_MS=packed[MODE_MATCH][row-1][col-1]&SCOREMASK;
				final int scoreFromSub_MS=packed[MODE_SUB][row-1][col-1]&SCOREMASK;
				...
				final int scoreFromDiag_DEL=Tools.max(packed[MODE_MATCH][row][col-1]&SCOREMASK, packed[MODE_SUB][row][col-1]&SCOREMASK);
				final int scoreFromDel_DEL=packed[MODE_DEL][row][col-1]&SCOREMASK;

				final int scoreFromDiag_INS=Tools.max(packed[MODE_MATCH][row-1][col]&SCOREMASK, packed[MODE_SUB][row-1][col]&SCOREMASK);
...
//				}
				...
				if(gap || !match || (scoreFromMatch_MS<=limit3 && scoreFromSub_MS<=limit3 && scoreFromDel_MS<=limit3 && scoreFromIns_MS<=limit3)){
					packed[MODE_MATCH][row][col]=subfloor;
				}else{//Calculate match and sub scores
					final int streak=(packed[MODE_MATCH][row-1][col-1]&TIMEMASK);
					...
					{//Calculate match/sub score
						...
						int scoreMS=prevMatch ? scoreFromMatch_MS+POINTSoff_MATCH2 : scoreFromSub_MS+POINTSoff_MATCH;
						...
						assert(score<=MAXoff_SCORE) : "Score overflow - use MSA2 instead";
//						packed[MODE_MS][row][col]=(score|prevState|time);
						packed[MODE_MATCH][row][col]=(score|time);
						assert((score&SCOREMASK)==score);
//						assert((prevState&MODEMASK)==prevState);
						assert((time&TIMEMASK)==time);
					}
				...
				if(gap || (scoreFromMatch_MS<=limit3 && scoreFromSub_MS<=limit3 && scoreFromDel_MS<=limit3 && scoreFromIns_MS<=limit3)){
					packed[MODE_SUB][row][col]=subfloor;
				}else{//Calculate match and sub scores
					final int streak=(packed[MODE_SUB][row-1][col-1]&TIMEMASK);
					...
					{//Calculate match/sub score
						...
						int scoreM=match ? subfloor : scoreFromMatch_MS+POINTSoff_MATCHSUB;
						int scoreS=scoreFromSub_MS+(streak==0 ? POINTSoff_SUB : streak<LIMIT_FOR_COST_3 ? POINTSoff_SUB2 : POINTSoff_SUB3);
						
						assert(false) : "Re-enable the mass-commented section.";
						/*
						if(ref1!='N' && call1!='N'){
							int scoreM=match ? subfloor : scoreFromMatch_MS+POINTSoff_MATCHSUB;
							int scoreS=scoreFromSub_MS+(streak==0 ? POINTSoff_SUB : streak<LIMIT_FOR_COST_3 ? POINTSoff_SUB2 : POINTSoff_SUB3);
...
//					assert((prevState&MODEMASK)==prevState);
				...
				if(col>=colStop){
					...
					if(row>1){
						packed[MODE_MATCH][row-1][col+1]=subfloor;
...
//				final boolean prevMatch=(row<2 || col<2 ? false : read[row-2]==ref[refStartLoc+col-2]);
				...
				if(gap){
					...
					final int scoreFromDiag=packed[MODE_MATCH][row-1][col-1]&SCOREMASK;
...
//							assert((prevState&MODEMASK)==prevState);
				...
				if(row<BARRIER_D1 || row>BARRIER_D2){
					...
					final int scoreFromDiag=packed[MODE_MATCH][row][col-1]&SCOREMASK;
...
//				if(gap || col<BARRIER_I1 || col>BARRIER_I2){
				if(gap || (row<BARRIER_I1 && col>1) || (row>BARRIER_I2 && col<BARRIER_I2b)){
					...
					final int scoreFromDiag=packed[MODE_MATCH][row-1][col]&SCOREMASK;
...
//			System.err.println("state="+state+", prev="+prev+", row="+row+", col="+col+", score="+scores[state][row][col]);
			...
			if(state==MODE_MATCH){
				if(time>1){prev=(byte)state;}
					...
					final int scoreFromDel=packed[MODE_DEL][row-1][col-1]&SCOREMASK;
					final int scoreFromIns=packed[MODE_INS][row-1][col-1]&SCOREMASK;
					if(scoreFromDiag>=scoreFromDel && scoreFromDiag>=scoreFromIns){prev=MODE_MATCH;}
...
//						out[outPos]='X';
				...
				if(time>1){prev=(byte)state;}
					...
					final int scoreFromDiag=packed[MODE_MATCH][row][col-1]&SCOREMASK;
					final int scoreFromDel=packed[MODE_DEL][row][col-1]&SCOREMASK;
					if(scoreFromDiag>=scoreFromDel){prev=MODE_MATCH;}
					else{prev=MODE_DEL;}
				}
				...
				if(time>1){prev=(byte)state;}
					...
					final int scoreFromDiag=packed[MODE_MATCH][row-1][col]&SCOREMASK;
					final int scoreFromIns=packed[MODE_INS][row-1][col]&SCOREMASK;
					if(scoreFromDiag>=scoreFromIns){prev=MODE_MATCH;}
					else{prev=MODE_INS;}
				}
...
//	public static final int TIMEMASK=(~((-1)<<TIMEBITS))<<TIMEOFFSET;
	...
	private static final byte MODE_MATCH=0;
	...
	private static final int BARRIER_I1=3;
