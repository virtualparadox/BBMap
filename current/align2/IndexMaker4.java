...
public class IndexMaker4 {
	...
	public static Block[] makeIndex(final int genome, int minChrom, int maxChrom, int k, int CHROMBITS, 
			...
			boolean COLORSPACE, boolean WRITE, boolean DISK_INVALID, Block[] index){
		Timer t=new Timer();
		t.start();
		
		MAX_CONCURRENT_BLOCKS=(Data.WINDOWS ? (WRITE ? 1 : Tools.max(1, Shared.THREADS/4)) : Tools.max(1, Shared.THREADS/4));
...
//				}
	...
	public static final String fname(int minChrom, int maxChrom, int k, int chrombits, boolean cs){
		return fname(minChrom, maxChrom, k, chrombits, cs, Data.GENOME_BUILD);
	}
	
	public static final String fname(int minChrom, int maxChrom, int k, int chrombits, boolean cs, int build){
		String suffix="_index_k"+k+"_c"+chrombits+"_b"+build+(cs ? "_cs" : "")+".block";
	...
	public static int MAX_CONCURRENT_BLOCKS=(Data.WINDOWS ? 1 : Tools.max(1, Shared.THREADS/4));
