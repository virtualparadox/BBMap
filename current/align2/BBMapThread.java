...
public final class BBMapThread extends AbstractMapThread{
	...
	public BBMapThread(ConcurrentReadStreamInterface cris_, int keylen_, 
...
//					System.err.println(i+": "+(ss.match==null ? "null" : new String(ss.match)));
				}else{
					ss.match=null;
				}
			}
...
//		assert(r.ambiguous()==ambi) : r;
		...
		if(MAKE_MATCH_STRING && r.numSites()>0){
			if(USE_SS_MATCH_FOR_PRIMARY && r.topSite().match!=null){
				...
				while(r.sites.size()>1 && r.topSite().score<r.sites.get(1).score){
					//				assert(false) : "Should be fixed:\n"+r.toText(false)+"\n"; //TODO:  It's still not fixed.  Fails in 1s_0i_0d_0u_400k.
		...
		if(r.sites!=null && r.mapScore<=0){//This came from BBMapThreadPacBio; not sure if needed for other modes
			if(!Shared.anomaly){
				System.err.println("Note: Read "+r.id+" failed cigar string generation and will be marked as unmapped.\t"+(r.match==null)+"\t"+r.mapScore+"\t"+r.topSite());
...
//			int delta=absdif(r.start+r.bases.length-1, r.stop);
//			if(delta>100){
...
//		if(r.numericID==2660){
//			verbose=msa.verbose=SiteScore.verbose=true;
//		}else{
//			verbose=msa.verbose=SiteScore.verbose=false;
//		}
		if(idmodulo>1 && r.numericID%idmodulo!=1){return;}
