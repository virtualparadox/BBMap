...
//					throw new RuntimeException(reads2+" does not exist.");
		...
		KmerCount7MTA.minQuality=5;
		KmerCount7MTA.minProb=0.5f;
...
//		}
		...
		if(threads<=0){
			if(auto){THREADS=Shared.THREADS;}
			else{THREADS=8;}
		}else{
			THREADS=threads;
		}
...
//		outstream.println();
		if(prefilter){
			prefilterArray=KmerCount7MTA.makeKca(in1, in2, extra, k, 2, 0, precells, prehashes, minq, true, tablereads, 1, buildStepsize, 1, 1, null);
		...
		kca=KmerCount7MTA.makeKca(in1, in2, extra, k, cbits, 0, cells, hashes, minq, true, tablereads, buildpasses, buildStepsize, 2, 2, prefilterArray);
		ht.stop();
...
//		outstream.println("\nTotal time:      \t\t"+t+"   \t"+String.format("%.2f", bases*1000000.0/(t.elapsed))+" kb/sec");
	...
	public static long downsample(ConcurrentReadStreamInterface cris, KCountArray kca, int k, long maxReads, 
		...
		if(COUNTUP){
			...
			int kbits=2*k;
			kcaup=KCountArray.makeNew(1L<<kbits, cells, bits, 0, 3, null); 
		}
...
//				histSum+=(x*(double)i);
	...
	private static int correctErrors(Read r, int[] cov, long[] kmers, KCountArray kca, final int k,
			final int low, final int high, final int mult, int maxToCorrect, int maxQual, boolean kmersAlreadyValid, boolean coverageAlreadyValid, long[] qhist, final boolean markOnly){
		assert(k<32);
		...
		if(!kmersAlreadyValid){kmers=r.toKmers(k, 0, kmers, false);}
		...
		if(CORRECT_FROM_LEFT){
			cfl=correctErrorsFromLeft(r, cov, kmers, kca, k, low, high, mult, maxToCorrect, maxQual, suffix, qhist, markOnly);
			if(cfl<0){
				//Failed correction.
				r.bases=copy;
				return cfl;
		...
		if(CORRECT_FROM_RIGHT && maxToCorrect>0){
			...
			cfr=correctErrorsFromRight(r, cov, kmers, kca, k, low, high, mult, maxToCorrect, maxQual, suffix, qhist, markOnly);
			if(cfr<0){
				//Failed correction.
				r.bases=copy;
				return cfr;
	...
	private static int markErrors(Read r, int[] cov, long[] kmers, KCountArray kca, final int k,
			final int low, final int high, final int mult, int maxToCorrect, boolean kmersAlreadyValid, boolean coverageAlreadyValid, long[] qhist){
		assert(k<32);
		assert(maxToCorrect>0) : "Don't do error correction with a maximum of 0 errors; it's a waste of time.";
		if(maxToCorrect<1){return 0;}
	 ...
	 * but the presence of discontiniuities indicates the presence of errors.
	 * @param cov
	 * @param low
	 * @param high
	 * @param mult
...
//					else if(q>0){quals[loc]=(byte)Tools.max(1, q/2);}
					...
					if(success){
						...
						//					r.toKmers(k, 0, kmers, false);
						//					generateCoverage(kca, k, cov, kmers, true);
						regenerateKmersAndCoverage(r, kmers, cov, kca, k, false);
					}else{
						uncorrected++;
						break;
					}
...
//		assert(false) : s+"\n"+Arrays.toString(split);
	...
	private static class ProcessThread extends Thread{
		...
		ProcessThread(ConcurrentReadStreamInterface cris_, KCountArray kca_, KCountArray kcaup_, int k_, 
				RTextOutputStream3 rosk_, RTextOutputStream3 rost_, RTextOutputStream3 rosl_, RTextOutputStream3 rosm_, RTextOutputStream3 rosh_, RTextOutputStream3 rosu_,
			...
			while(reads!=null && reads.size()>0){
				for(int rnum=0; rnum<reads.size(); rnum++){
					...
					if(!TRIM_AFTER_MARKING && (TRIM_LEFT_THIS_PASS || TRIM_RIGHT_THIS_PASS)){
						if(r!=null){basesTrimmed+=TrimRead.trimFast(r, TRIM_LEFT_THIS_PASS, TRIM_RIGHT_THIS_PASS, TRIM_QUALITY, 1);}
					...
					if(r!=null && r.bases!=null){
						...
						if(r.bases.length>=k){
							...
							kmers1=r.toKmers(k, kca.gap, kmers1, false);
							cov1=generateCoverage(kca, k, cov1, kmers1, k<32);
							
							if(CORRECT_ERRORS_THIS_PASS){
								if(MARK_ERRORS_ONLY){
									ec1=markErrors(r, cov1, kmers1, kca, k, EC_LTHRESH, EC_HTHRESH, ERROR_CORRECT_RATIO, MAX_ERRORS_TO_CORRECT, true, true, qhist);
									errorsMarked+=ec1;
									if(ec1>0){marked1=true;}
								}else{
									ec1=correctErrors(r, cov1, kmers1, kca, k, EC_LTHRESH, EC_HTHRESH, ERROR_CORRECT_RATIO, MAX_ERRORS_TO_CORRECT, MAX_QUAL_TO_CORRECT, true, true, qhist, MARK_ERRORS_ONLY);
									if(ec1>=0){
										errorsCorrected+=ec1;
									}else{
										uncorrectable1=true;
										if(MAX_ERRORS_TO_CORRECT>0){regenerateKmersAndCoverage(r, kmers1, cov1, kca, k, false);}
								...
								if(TRIM_AFTER_MARKING){
									if(marked1 || TRIM_EVEN_IF_NO_ERRORS_DETECTED){
										basesTrimmed+=TrimRead.trimFast(r, TRIM_LEFT_THIS_PASS, TRIM_RIGHT_THIS_PASS, TRIM_QUALITY, 1);
									}
								}
							}
							sortCoverageAndIncrementHistogram(cov1);
							int[] cov=cov1;
					...
					if(r2!=null && r2.bases!=null){
						...
						if(r2.bases.length>=k){
							if(verbose){outstream.println();}
							kmers2=r2.toKmers(k, kca.gap, kmers2, false);
							cov2=generateCoverage(kca, k, cov2, kmers2, k<32);
							
							if(CORRECT_ERRORS_THIS_PASS){
								if(MARK_ERRORS_ONLY){
									ec2=markErrors(r2, cov2, kmers2, kca, k, EC_LTHRESH, EC_HTHRESH, ERROR_CORRECT_RATIO, MAX_ERRORS_TO_CORRECT, true, true, qhist);
									errorsMarked+=ec2;
									if(ec2>0){marked2=true;}
								}else{
									ec2=correctErrors(r2, cov2, kmers2, kca, k, EC_LTHRESH, EC_HTHRESH, ERROR_CORRECT_RATIO, MAX_ERRORS_TO_CORRECT, MAX_QUAL_TO_CORRECT, true, true, qhist, MARK_ERRORS_ONLY);
									if(ec2>=0){
										errorsCorrected+=ec2;
									}else{
										uncorrectable2=true;
										if(MAX_ERRORS_TO_CORRECT>0){regenerateKmersAndCoverage(r2, kmers2, cov2, kca, k, false);}
										if(MARK_UNCORRECTABLE_ERRORS){
											ec2=markErrors(r2, cov2, kmers2, kca, k, EC_LTHRESH, EC_HTHRESH, ERROR_CORRECT_RATIO, MAX_ERRORS_TO_CORRECT, true, true, qhist);
											errorsMarked+=ec2;
											if(ec2>0){marked2=true;}
							...
							sortCoverageAndIncrementHistogram(cov2);
							int[] cov=cov2;
					...
					if(RENAME_THIS_PASS){
						if(r2==null){
							final String s="id="+r.numericID+",d1="+depthAL1+(CORRECT_ERRORS_THIS_PASS ? ",e1="+(ec1<0 ? -ec1 : 0) : "");
							r.id=s;
							if(EA){
								int[] quad=parseDepth(r.id, null);
							...
							final String s="id="+r.numericID+",d1="+depthAL1+",d2="+depthAL2+(CORRECT_ERRORS_THIS_PASS ? ",e1="+(ec1<0 ? -ec1 : 0)+",e2="+(ec2<0 ? -ec2 : 0) : "");
							r.id=s+" /1";
							r2.id=s+" /2";
							if(EA){
								int[] quad=parseDepth(r.id, null);
...
//						targetDepth=(int)((TARGET_DEPTH_BAD_LOW*(long)lowcount+TARGET_DEPTH_BAD_HIGH*(totalcount-(long)lowcount))/totalcount);
					...
					final int lowerdepthAL=(depthAL1>=0 ? (depthAL2>=0 ? Tools.min(depthAL1, depthAL2) : depthAL1) : depthAL2); //The lower of depth1 and depth2
					final int lowertruedepth=(r2==null ? truedepth1 : Tools.min(truedepth1, truedepth2));
					long coin=0;
					if(lowerdepthAL>maxDepth && (error1 || error2 || !DISCARD_BAD_ONLY)){
						if(r.rand<0){
							coin=randy.nextLong(lowerdepthAL)+1;
						}else{
							coin=((long)(r.rand*lowerdepthAL))+1;
						}
					}
					...
					boolean toss=(lowerdepthAL<0 || coin>targetDepth || (r!=null && r.bases.length<minlength) || (r2!=null && r2.bases.length<minlength));
					if(TOSS_ERROR_READS && (error1 || error2)){
						if(SAVE_RARE_READS && lowerdepthAL<=targetDepth && lowerdepthAL>=HTHRESH){
							//do nothing
					...
					if(TOSS_BY_LOW_TRUEDEPTH && !SAVE_RARE_READS && lowertruedepth<MIN_DEPTH && (!REQUIRE_BOTH_BAD || (mintruedepth1<MIN_DEPTH && mintruedepth2<MIN_DEPTH))){
						toss=true;
					}
...
//					}else if((depth1<0 || depth1>HIGH_BIN_DEPTH) && (depth2<0 || depth2>=HIGH_BIN_DEPTH)){
						...
						if(highList!=null){highList.add(r);}
					}else{
			...
			while(reads!=null && reads.size()>0){
				for(int rnum=0; rnum<reads.size(); rnum++){
					...
					if(r!=null && r.bases!=null){
						...
						if(r.bases.length>=k){
							if(verbose){outstream.println();}
							kmers1=r.toKmers(k, kca.gap, kmers1, true);
							k1valid=true;
						}
					}
					if(r2!=null && r2.bases!=null){
						...
						if(r2.bases.length>=k){
							if(verbose){outstream.println();}
							kmers2=r2.toKmers(k, kca.gap, kmers2, true);
							k2valid=true;
						}
					}
...
//					System.err.println("Adding list "+ln.id+" of length "+reads.size());
		...
		private final int[] getSortedCoverageAndIncrementHistogram(Read r, int[] cov, long[] kmers, 
				boolean kmersAlreadyValid, boolean kmersAlreadyCanonical, boolean coverageAlreadyValid){
			assert(r!=null && r.bases!=null && r.bases.length>=k) : r;
			
			if(!coverageAlreadyValid){
				if(!kmersAlreadyValid){kmers=r.toKmers(k, kca.gap, kmers, false);}
...
//			assert(sum2==cov.length) : sum2+", "+cov.length+", "+last+", "+sum;
		...
		private final RTextOutputStream3 rost;
		/** Stream for low-count reads */
		private final RTextOutputStream3 rosl;
		/** Stream for mid-count reads */
		private final RTextOutputStream3 rosm;
		/** Stream for high-count reads */
		private final RTextOutputStream3 rosh;
		/** Stream for reads with uncorrectable errors */
		private final RTextOutputStream3 rosu;
...
//		private final java.util.Random randy=new java.util.Random();
	...
	private static int MIN_DEPTH=6;
